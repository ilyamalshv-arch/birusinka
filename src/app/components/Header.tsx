...
const Header = () => {
  ...
  return (
    <>
      <header>
        <Container className="flex h-[90px] items-center justify-between max-3xl:h-[50px]">
          <button
            type="button"
            aria-label="Открыть меню"
            onClick={() => setMenuOpen(true)}
            className="hidden flex-col justify-center gap-[5px] max-3xl:flex"
          >
          ...
          <div className="flex items-center gap-[54px]">
            <a href="/" aria-label="Бирюсинка — на главную">
              <Image
                ...
                height={28}
                priority
                className="h-[28px] w-[237px] max-3xl:h-[13px] max-3xl:w-[111px]"
                alt="Бирюсинка"
              />
            </a>
            <nav className="flex items-center gap-[30px] max-3xl:hidden">
              {mainNav.map((item) => (
                <div key={item.label} className="group relative flex h-[90px] items-center">
                  <a
                    href={item.href}
                    className={`${montserratAlt.className} text-[20px] font-medium leading-[24px]`}
                  >
                    {item.label}
                  </a>
                  <div className="absolute left-0 top-full z-50 hidden min-w-[249px] flex-col gap-[16px] rounded-[10px] bg-white p-[20px_24px] shadow-[0_10px_30px_rgba(0,0,0,0.12)] group-hover:flex">
                    {item.links.map((link) => (
                      <a
                        key={link}
                        href="#"
                        className="whitespace-nowrap text-[18px] font-normal leading-[22px] text-black hover:text-brand"
                      >
                        {link}
                      </a>
                    ))}
          ...
          <div className="flex items-center gap-[40px]">
            <a href="#" className="flex items-center gap-[10px] max-3xl:hidden">
            ...
            <a href="#" aria-label="Избранное" className="max-3xl:hidden">
              <Image
            ...
            <a href="#" aria-label="Корзина">
              <Image
                ...
                width={29}
                height={29}
                className="h-[29px] w-[29px] max-3xl:h-[18px] max-3xl:w-[18px]"
                alt=""
              />
            </a>
          </div>
            ...
            {loggedIn ? (
              <div className="flex items-center gap-[10px]">
                ...
                <button
                  ...
                  className="rounded-[8px] bg-brand px-[20px] py-[8px] text-[14px] font-medium text-white"
                >
                  Выйти
                </button>
              </div>
          ...
          <div className="flex flex-col gap-[14px] pt-[8px]">
            {topBarLinks.map((link) => (
              <a
                key={link.label}
                href={link.href}
                className="text-[18px] font-light leading-[22px]"
              >
                {link.label}
              </a>
            ))}
